<header class="lg:ml-64 fixed top-0 right-0 left-0 lg:left-64 z-30 bg-white/80 backdrop-blur-md h-16 px-5 sm:px-8 flex items-center justify-between border-b border-outline-variant/15">
    ...
    <div class="hidden lg:block">
        <h1 class="text-base font-bold text-on-surface">@yield('page_title', 'Seeker Portal')</h1>
    </div>

    <div class="flex items-center gap-3">
        <a
            class="hidden sm:inline-flex gradient-primary text-white px-5 py-2 rounded-full font-bold text-xs shadow-md hover:scale-105 transition-transform"
            href="{{ route('browse') }}"
        >
            Browse Jobs
        </a>
